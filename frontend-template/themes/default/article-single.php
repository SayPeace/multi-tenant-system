...
<article>
    <div class="card">
        ...
        <div class="article-meta" style="padding: 15px; background: #f9f9f9; border-radius: 4px; margin-bottom: 25px;">
            ...
            <?php endif; ?>
            <div><strong>Views:</strong> <?= number_format($article->view_count) ?></div>
            <div><strong>Downloads:</strong> <?= number_format($article->download_count) ?></div>
        </div>
        ...
        <div style="margin-top: 30px; padding: 15px; background: #f0f7ff; border-radius: 4px;">
            ...
            <p style="font-size: 0.9rem;">
                ...
                $authorStr = count($authorList) > 2
                    ? $authorList[0] . ' et al.'
                    : implode(' & ', $authorList);
                $year = date('Y', strtotime($article->published_at));
                ?>
